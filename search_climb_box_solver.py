...
class MCTSTaskSequence(MCTSBase):
    ...
    def evaluate(self, simulation_path : list) -> float:
        ...
        W_HEIGHT = 1.
        max_height = HEIGHT + config_gait.nom_height
        reward *= np.exp(- W_HEIGHT * np.mean(np.abs(max_height - q_sol[-1, 2])))
        # reward *= np.exp(- W_HEIGHT / 3 * np.mean(np.abs(max_height - q_sol[:, 2])))
        ...
        if reward > self.max_reward:
            print(simulation_path[-1], reward, q_sol[-1, 2], n_qp)
...
mcts = MCTSTaskSequence(graph, C=15., increase_res_it=increase_res_it)
